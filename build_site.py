...
def build_index():
    ...
    body = (
        ...
        "mode.</p></div>"
        "<p class='muted'>Two things temper this. The boundary is <b>the vendor's to draw</b>: they decide what sits "
        "inside it, and can draw it wider to bring more of the product under validation, at the cost of a larger and "
        "more expensive evaluation. And what is examined inside the boundary is <b>more than the algorithms</b>. The "
        "validation grades the module across roughly a dozen requirement areas, and as the security level rises the "
        "scope expands outward from the mathematics into physical protection, tamper detection and response, and the "
        "controls around the keys, the areas the levels below describe.</p>"
        ...
        "<div class='cols3'>"
        "<div class='panel'><h3>“FIPS certified” means the module, not the product</h3><p>A product can embed a validated "
        ...
        "separate protection domain, so compromising the application does not directly hand over the key. Just as often, "
        "the reason to introduce one is the <b>administrative boundary</b> it creates. The device runs its own identity "
        "and authentication subsystem, frequently built on smart cards, with its own operators and roles, so using or "
        "managing the keys means crossing into a separate domain of control. That is as much a defense against insider "
        "misuse, and a way to enforce separation of duties, as it is a barrier to an outside attacker. But that only "
